...
public class Fuel {
    ...
    public void crashCheck(PlayerRocket playerRocket){
        if(playerRocket.getX() + playerRocket.getRocketImgHeight() > fuelX && playerRocket.getX() < fuelX + fuelImgWidth 
        		&& playerRocket.getY() + playerRocket.getRocketImgHeight() > fuelY && playerRocket.getY() < fuelY + fuelImgHeight){
            refueling();
            relocationFuel();
        }
    }
